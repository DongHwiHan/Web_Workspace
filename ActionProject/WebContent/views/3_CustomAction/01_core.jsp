...
- 변수를 선언하고 초기갑슬 대입해두는 기능을 제공
- 해당 변수를 어떤 scope에 담아둘껀지 지정 가능함(생략시 기본적으로 pageScope에 담김)
=> 즉, 해당 scope영역에 setAtteribute라는 메소드를 이용해서 key + value형태로 데이터를 담아놓는것과 동일한 형태다
...
<!-- 
	변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳으로 순차적으로 찾아지게됨. 티가나지는 않지만 속도가 조금 느려질수가 있다.
...
- 데이터를 출력하고자할떄 사용하는 태그
- 기본값 : value에 출력하고자하는 값이 없을경우 기본값으로 출력할 내용물을 기술(생략가능)
...
<%
	...
	list.add(new Person("민","남","강"));
	list.add(new Person("경","과","사"));
	list.add(new Person("민","여","님"));
	...
	<c:choose>
		...
		<c:forEach var="p" items="${ requestScope.list }" varStatus="status">
				<tr align="center">
					...
					<td>${ p.name }</td> <!-- ${list[status.index].name} -->
					<td>${ p.gender }</td>
					<td>${ p.job }</td>
				</tr>
			</c:forEach>
		...
		<!-- 기본 반복문으로 컬렉션배열 반복시키기 -->
			<c:forEach var="i" begin="0" end="<%= list.size()-1 %>">
				<tr align="center">
					...
					<td>${ list[i].name }</td> <!-- ${list[status.index].name} -->
					<td>${ list[i].gender }</td>
					<td>${ list[i].job }</td>
				</tr>
			</c:forEach>
...
- 구분자를 통해서 분리된 각가의 문자열에 순차적으로 접근하면서 반복 수행
...
&lt;c:url var="변수명" value="요청할url"&gt;
	...
	&lt;c:param name="키값" value="밸류값"/&gt;
&lt;/c:url&gt;
- url경로를 생성을하고, 쿼리스트링을 정의할수 있는 태그
- 넘겨줘야할 쿼리스트링이 길경우 사용하면 편리하고,
- 특정조건에 따라 파라미터값을 다르게 보내야할때 자주 사용됨
</pre>
