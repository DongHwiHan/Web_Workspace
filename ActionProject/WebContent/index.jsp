...
<body>
	...
	<p>
		...
		javascript의 \${자바스크립트변수 }를 쓰고싶다면 페이지 지시어중 isELIgnored속성을 true로 변경해주면 됨.
	</p>
	
	<h3>1. EL 기본 문구</h3>
	<a href="/action/el.do">10_EL</a>
	...
	<!-- 
		* JSP를 이루는 구성인자
		...
		1. JSP 스크립팅원소 : JSP페이지 안에서 자바코드를 직접 기술할수 있게 하는 기술
		   EX) 선언무, 스크립틀릿, 표현식, 지시어 ..
		...
		3. JSP 액션태그 : XML기술을 이용해서 기존 jsp문법을 확장하는 기술을 제공하는 태그
				- 표준액션태그(Standard Action Tag) : JSP페이지에서 바로 사용가능(별도의 라이브러리가 필요 없다)
													표준액션태그는 모든 태그명 앞에 jsp: 이라는 접두어가 붙음.
				- 커스텀액션태그(Custom Action Tag) : JSP페이지에서 바로 사용불가능(별도의 라이브러리 연동 해줘야함)
													커스텀액션태그는 모든 태그명 안에 별도의 접두어가 붙음(c:, fn:, fmt:)
													재공되고 있는 대표적인 유용한 라이브러리가 (JSTL)
	 -->
	...
	<h3>1. 표준 액전 태그</h3>
	<p>
		JSP페이지에서 별도의 라이브러리 연동 없이 바로 사용 가능함 <br>
		태그 앞에 jsp:접두어가 붙음
	</p>
