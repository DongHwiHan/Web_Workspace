...
public class BoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request) ) {
			...
			// 1_1. 전송파일 용량제한(int maxSize => byte단위으 값을 기술해야함.) 저희는 10Mbyte로 제한할 예정
			/*
			 * 단위정리
			 ...
			 * 다만 WebContetns 폴더로부터 board_upfiles폴더까지의 경로는 매개변수로 제시해줘야한다.
			 ...
			 *  MutipartRequest multi = new MultipartRequest(request 객체, 저장할 폴더 경로, 용량제한, 인코딩값, 
			 *    파일명을 수정시켜주는 객체);
			 *   
			 *  위 구문 한줄 실행만으로 넘어온 첨부파일들이 해당 폴더에 업로드됨
			 *  그리고 사용자가 올린 파일명은 그대로 해당폴더에 업로드 하지 않는게 관례
			 *  - 같은 파일명이 있을경우 덮어씌워지거나 에러가 날수 있꼬, 한글/ 특수문자/ 띄어쓰기가 포함된 파일명일 경우 사용하는
			 *  서버에따라 에러가 발생할수 있기 때문. 따라서 파일명은 반드시 변환시켜줘서 업로드할 예정
			 *  
			 *  기본적으로 파일명 수정작업을 해주는 객체
			 *   - DefaultFilenamePolicy 객체(cos.jar에서 제공)
			 *   - 내부적으로 rename()이라는 메소드가 실행이되면서 파일명이 수정됨.
			 *   - 기본적으로 동일한 파일명이 존재할 경우 파일명 뒤에 카운팅된숫자를 붙여서 유니크한 파일명을 만듬
			 *     ex) aaa.jpg, aaa1.jpg, aaa2.jpg ... 
			 *   
			 *   - 절대 파일명이 안겹치게끔 rename해볼예정
			 *   - 즉, DefaultFilenamePolicy객체는 사용하지 않음
			 *   내 입맛대로 나만의 파일명 생성 규칙을만들어서 파일이름을 바꿔주는 객체를 만들어보기.
			 * 
			 * */
			 ...
			 Attachment at = null;// 처음에는 null값으로 초기화시키고 , 실제로 사용자가 첨부파일을 업로드 했을때만 객체생성
			
			 // 첨부파일이 있을경우 원본명을 반환해주고, 없을경우 null을 반환해줌
			 if(multi.getOriginalFileName("upfile") != null) {
				 at = new Attachment();
				 at.setOriginName( multi.getOriginalFileName("upfile")  );// 원봉명
