...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 필요한 데이터를 전달받고 실제 Board, Attachment테이블에 Update쿼리문을 실행
		
		// 1. 전송된데이터 input file이 포함된경구 enctype="multipart/form-data"로 전송했을것.
		if(ServletFileUpload.isMultipartContent(request)) {
			
			// 1_1. 전송파일 용량제한(10mbtye)
			...
			if(multi.getOriginalFileName("upfile") != null) {
				 ...
				 if(multi.getParameter("originFileNo") != null) {
					 ...
					 //기존의 첨부파일을 삭제
					 new File(savePath+multi.getParameter("changeName")).delete();
					 
				 }else {
					 // 기존에 첨부파일 없는경우
					 // Attachment테이블의 정보를 insert
			...
			// case3 : 새로운 첨부파일이 있는경우(o), 기존에는 첨부파일이 없는경우(x) -> update(x), insert(o)
