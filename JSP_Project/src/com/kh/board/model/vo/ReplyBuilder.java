...
public class ReplyBuilder {
	 ...
	 *  - 생성자에 인자가 너무 많이 있을때 고려해서 사용해보면됨.(필드가 몇개없는 간단한 생성자는 오히려 사용하지 않는것을 권장)
	 *  
	 *  기존 생성방식 
	 *  	1. 생성자를 이용한 객체생성
	 *  	   객체를 생성할때 생성자에 매개변수로 데이터를 넣어줘서 객체를 생성하는 방식.
	 *         단점? 1) 인자들이 많을수록 생성자가 많아질수 있다.
	 *              ex) Member 클래스의 생성자로(로그인 했을때의 정보를 담을수있는객체 ,회원가입용 생성자, 로그인요청 생성자 등등)
	 *              생성자는 딱 필요로 하는 데이터만 담고있는게 좋으므로 그에맞는 생성자가 많이 필요해지게 된다.
			...
			2. setter함수를 이용한 객체생성
			    생성자를 이용한 객채생성방법보다 2번 단점이 보완되었다. 매개변수의 정보를 설명할 수 있기때문에 가독성은 확보가 되나 
