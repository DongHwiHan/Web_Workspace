...
<script>
   // 웹소켓 서버에 연결하기 
   // WebSocket 객체 생성하기 192.168.30.170
   <%-- const socket = new WebSocket("ws://ip:port/<%=request.getContextPath() %>/chatting.do") --%>   
   const socket = new WebSocket("ws://192.168.120.56:8081/<%= request.getContextPath() %>/chatting2.do")
   // https://ip : 포트번호 / ~~
   // http -> ws: ~
   // https -> wss: ~
   ...
   // 2. 웹소켓 서버에서 sendText, sendObject 메소드를 실행하면 실행되는 함수
