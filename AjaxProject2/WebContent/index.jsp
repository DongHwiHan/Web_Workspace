...
<body>
	...
	<p>
		Asynchronous JavaScript And Xml의 약자 서버로부터 데이터를 가져와 전체 페이지를 새로고침하지 않고
		일부만 로드할수 있게 하는 기법<br> 우리가 기존에 a태그로 요청하거나 form를 통해 요청했던 방식은 동기식
		요청방식<br> => 응답페이지가 돌아와야 볼수있으며 응답페이지가 호출되기전까지 아무요청하지못함<br>
		비동기식 요청을 보내기 위해서는 AJAX라는 기술이 필요함 <br> <br> * 동기식 / 비동기식 <br>
		- 동기식 : 요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업 가능함<br> 만약 서버에서 호출된
		결과까지의 시간이 지연되면 계속 기다려야함(흰화면으로 보이게됨) <br> 전체 페이지가 리로드됨(새로고침 즉,
		페이지가 기본적으로 한번 깜빡거림)<br> <br> - 비동기식 : 현재 페이지를 그대로 유지하면서 중간중간
		추가적인 요철을 보내줄 수 있음<br> 요청을 한다고해서 다른페이지로 넘어가지 않음(현재 페이지 유지)<br>
		요청을 보내놓고 그에 해당하는 응답이 돌아올때 까지 현재 페이지에서 다른 작업을 할수있다<br> ex) 아이디
		중복체크기능, 검색어 자동완성기능 * 비동기식의 단점<br> - 현재 페이지에 지속적으로 리소스가 쌓임 => 페이지가
		느려질수 있음 <br> - 페이지내 복잡도가 크게 증가함 -> 여러발생시 대터가 어려워질수 있음 <br> -
		요청 후 들어온 응답데이터를 가지고 현재 페이지에 새로운 요소를 만들어서 뿌려줘야되는데, DOM요소를 새로이 만들어내는 구문을
		잘숙지해야함<br> <br>
	</p>
	...
	<pre>
		...
		* 부수적인 속성 
		...
		- dataType : response로 오는 데이터의 자료형설정(갑슬 추가안하면 스마트하게 판단함)
					text : String데이터
					xml : 트리형태의 구조
		...
		- crossDomain : 타 도메인 호출 가능 여부 (기본갑 false)
		- dataFilter : response데이터를 받았을때 정상적인 값을 return할 수 있도록 데이터와 데이터 타입 설정
		- global : 기본 이벤트 사용여부 => 퍼버링같이 시작과 끝을 나타낼때, 선처리 작업시 사용함.
		- password : 서버의 접속권한이 필요한 경우 설정
		- processData : 서버로 보내는 값에 대한 형태 설정 여부(기본데이터 원하는 경우 false)
	...
	이름 :
	<input type="text" id="input2_1">
	<br> 나이 :
	<input type="text" id="input2_2">
	<br>
	<button onclick="test2();">전송</button>
	<br> 응답 :
	<label id="output2">응답 대기중 ...</label>
	...
	<button onclick="test4();">회원정보</button>
	...
	<script>
		function sendFile(){
			// 파일 전송시에는 FormData라는 객체를 생성해서 파일으 추가시켜줘야함
			let form = new FormData();
//			console.log($("#upfile")[0], $("#upfile")[0].files)
