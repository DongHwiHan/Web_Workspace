...
public class MemberDeleteController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 방법 1. session영역에 담겨잇는 회원객체로부터 뽑아내기
		// 방법 2. input type="hidden" 회원정보를 숨겨서 요청시 함께 전달하기
		
		
		HttpSession session = request.getSession();
		
		if(result >0 ) { // 성공시 -> 메인페이지 alert, 로그아웃처리
			//먼저 세션안에 담긴 데이터를 다 날린 후에 alertMsg만 따로 추가함
			// session.invalidate();
			...
			// invalidate() 메소드는 사용시 세션이 만료되어 안에들어간 데이터가 모두 날라감 -> alert메세지 호출할 수 없음
			// removeAttribute("loginUser") : 로그인한 사용자의 정보를 지워줌
			
			response.sendRedirect(request.getContextPath());
			
			// 로그아웃 처리를 로그아웃 서블릿에게 위임하는 방법
