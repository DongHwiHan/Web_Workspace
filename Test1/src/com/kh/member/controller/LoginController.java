...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - request : 서버로 요청할 떄의 정보들이 담겨있음 (요청시 전달한 값, 전송방식 등)
		 * - response : 요청에 대해 응답할때 필요한 객체
		 * 
		 * <GET 방식과 POST방식의 차이>
		 * - GET 방식 : 사용자가 입력한 값들이 url노출 o / 데이터의 길이제한 o / 즐겨찾기 기능상 편리함 
		 * - POST 방식 : 				  url노출 x / 데이터의 길이제한 x / 대신 timeout이 존재
		 * 
		 * 
		 * 
		 */
		...
		/*
		 * 응답 페이지에 전달한 값이 있을경우 값을 어딘가에 담아야함(담아줄 수 있는 Serlvet Scope의 내장객체 4종류)
		 * 
		 * 1) application : application에 담은 데이터는 웹 애플리케이션 전역에서 다 꺼내 쓸 수 있음
		 * 2) session : session에 담은 데이터는 모든 jsp와 servlet에서 꺼내 쓸 수 있음
		 * 				한번 담은 데이터는 내가 직접 지우기전까지, 서버가 멈추기 전까지, 브라우저가 종료되기 전까지 접근해서 꺼내 쓸수있음
		 * 3) request : request에 담은 데이터는 해당 request를 포워딩한 응답 jsp에서만 꺼내 쓸 수 있다.
		...
		if(loginUser == null) { // 로그인 실패 => 에러페이지 응답
			...
			// 포워딩 방식 : 해당 결로로 선택된 뷰가 보여질 뿐 url은 변경되지 않는다.(요청했을때의 url(login.me)이 그대로 남아있음)
			...
			// Servlet에서 jsp내장객체인 SESSION에 접근하고자 한다면 session 객체를 얻어와야함.
			...
			// 1. 포워딩방식 응답 뷰 출력하기
			// 선택된 jsp가 보여질뿐 url에는 여전히 현재 이 서블릿에 대한 매핑값이 남아있을것
			// localhost:8082/jspproject/login.me
			// url에 내용이 그대로 남아있는 상태에서 새로고침(f5)누르게 되면 계속 로그인 요청을 보내게될것.
